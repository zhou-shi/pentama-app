    ...
    switch (action) {
      case 'UPDATE_STAGE': {
        ...
        return NextResponse.json({
          message: `Status berhasil diubah menjadi ${stage}`,
        });
      }
      ...
      case 'APPROVE_SUBMISSION': {
        ...
        await db.runTransaction(async (t) => {
          ...
          if (examiners?.examiner1Uid) {
            const examiner1Ref = db
              .collection('users')
              .doc(examiners.examiner1Uid);
            t.update(examiner1Ref, {
              'lecturer.examinerCount': FieldValue.increment(1),
            });
          }
          if (examiners?.examiner2Uid) {
            const examiner2Ref = db
              .collection('users')
              .doc(examiners.examiner2Uid);
            t.update(examiner2Ref, {
              'lecturer.examinerCount': FieldValue.increment(1),
            });
          }
        });
      ...
      case 'UPDATE_PERSONNEL': {
        ...
        const updateField =
          personnelType === 'supervisors'
            ? { supervisors: personnelData }
            : { examiners: personnelData };
        await db
          .collection(collectionName)
          .doc(docId)
          .update({ ...updateField, updatedAt: new Date().toISOString() });
        return NextResponse.json({
          message: `${personnelType} berhasil diperbarui!`,
        });
      }
      ...
      case 'UPDATE_ROOM': {
        ...
        await db
          .collection('ruang')
          .doc(id)
          .update({ ...dataToUpdate, updatedAt: new Date().toISOString() });
      ...
      case 'PENDING_SUBMISSION': {
        ...
        const newDateTime = new Date(
          startTime.getTime() + (durationInMinutes as number) * 60000
        );
        const newScheduleData = {
          date: newDateTime.toLocaleDateString('id-ID', {
            weekday: 'long',
            day: 'numeric',
            month: 'long',
            year: 'numeric',
          ...
          time:
            newDateTime.toLocaleTimeString('id-ID', {
              hour: '2-digit',
              minute: '2-digit',
            }) + ' WIB',
          room: payload.room,
        ...
        const adminNote = `Jadwal ditunda pada ${new Date().toLocaleString(
          'id-ID',
          { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }
        )} ${new Date().toLocaleTimeString('id-ID', {
          hour: '2-digit',
          minute: '2-digit',
        }) } WIB. Dikarenakan: ${reason}. Jadwal baru akan dibuat setelah ${durationInMinutes} menit dari sekarang.`;
